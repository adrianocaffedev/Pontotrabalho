...
const AbsenceModal: React.FC<AbsenceModalProps> = ({ isOpen, onClose, onSave, initialDate }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in text-left">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden border border-white/20 dark:border-slate-700 transition-colors">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-xl shadow-inner transition-colors">
            <button
                ...
                className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all duration-200 ${type === 'ABSENCE' ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
            >
                Falta
            </button>
            <button
                ...
                className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all duration-200 ${type === 'DELAY' ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-white shadow-sm' : 'text-slate-400 hover:text-slate-600 dark:hover:text-slate-300'}`}
            >
                Atraso
            </button>
          </div>
